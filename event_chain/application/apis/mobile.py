...
def parse_wallet(request):
    user_did = request.args.get('userDid')
    user_pk = forge_utils.multibase_b58decode(
            request.args.get('userPk'))
    if not user_did:
        logger.error("No userDid")
        return
    elif not user_pk:
        logger.error('No userPk')
        return
    else:
        user_address = user_did.split(":")[2]
        logger.debug(f"user address parsed from request {user_address}")
        return models.Wallet(did=user_did, pk=user_pk, address=user_address)
...
def poke():
    try:
        if request.method == 'GET':
            ...
            wallet=parse_wallet(request)

            poke_tx = controllers.gen_poke_tx(wallet)

            did_request_params = {
                'user_did': wallet.did,
                'tx': poke_tx,
                'url': config.SERVER_ADDRESS + url_for('api_mobile.poke'),
                'description': 'Poke and get reward.',
                'action': 'responseAuth',
                'workflow': 'poke',
            }
            response = controllers.require_sig(**did_request_params)
            logger.debug('did auth response: {}'.format(did_request_params))
            return response
            ...
            if hash:
                js = json.dumps({'hash': hash,
                                 'tx': forge_utils.multibase_b58encode(poke_tx.SerializeToString())})
                logger.debug('success response: {}'.format(str(js)))
                resp = Response(js, status=200, mimetype='application/json')
                return resp
            else:
